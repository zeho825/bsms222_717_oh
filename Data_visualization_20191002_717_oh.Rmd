---
title: "Data_visualization_20191002_717_oh"
date: "`r Sys.Date()`"
output:
  rmdformats::readthedown:
    highlight: kate
---
# 6. Data visualization in practice {#gapminder}
이 챕터서는 배운 ggplot 코드로 겁나 쉽게 insightful하고 aesthetically하게 플롯을 만들어 볼거에요.

 We will implement what we learned in Chapters \@ref(ggplot2) and \@ref(other-geometries) and learn how to augment the code to perfect the plots. 
...
## 6.1 Case study: new insights on poverty
...
### 6.4.1 Labels instead of legends 
...
past_year <- 1970
gapminder %>% 
  filter(year == past_year & !is.na(gdp)) %>% #past_year자리에 걍 1970박아도 ㄱㅊ
...
### 6.5.3 Transform the values or the scale? 
...
그니까 걍 `scale_x_continuous(trans = "log숫자")`쓰셈.
...
## 6.7 Comparing multiple distributions with boxplots and ridge plots
...
gapminder <- gapminder %>% 
  mutate(group = case_when(
    region %in% c("Western Europe", "Northern Europe","Southern Europe", 
                    "Northern America", 
                  "Australia and New Zealand") ~ "West", #다음 벡터 있는 놈들 물결표로 포함 시켜버림.
...
### 6.7.3 Example: 1970 versus 2010 income distributions
...
### 6.7.4 Accessing computed variables 
...
## 6.8 The ecological fallacy and importance of showing the data
...
### 6.8.1 Logistic transformation {#logit}
