---
title: "Statiscal_inference_20191125_717_oh"
...
(머신 러닝에서 파란 커브줄 어케 생기는지 보고...)
일단은 Confidence interval의 가장 관련된 커브 주변 shaded된 부분을 생각해 봅시다.
...
polls_s <- polls %>%
  ...
  mutate(se_hat = sqrt(x_hat*(1-x_hat)/polls$samplesize)) %>% 
...
```{r}
two_by_two <- data.frame(record = c('win', 'loss'), important_events = c(8, 9), normal_events = c(350-8, 150-9))
