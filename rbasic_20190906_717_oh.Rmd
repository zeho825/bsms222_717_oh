...
### 3.2.1 Objects지정: `아무 문자 <- `
아무 문자로 Objects지정 가능함, `variables: 변수`를 지정하는 것임.
ex)  `a`, `b` and `c`. 
각각의 `variables: 변수`는 `numeric: 수`의 값을 내장함  
...
c <- -1
```
__**R**에서 변수를 지정할때에는 `<-`를 주로 씀, `=`는 주로 숫자를 만들때라...__

여기서 `a` variable을 보려먼 걍 `a`치면 그만. 만약 지정 안된 `d`를 치게 되면..
```{r}
a
d
```
...
### 3.2.2 The workspace: `ls()` 모든 variables보기
