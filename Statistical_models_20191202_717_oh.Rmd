---
title: "Statistical_models_20191202_717_oh"
...
Forecasters also use models to describe variability(변동) at different levels. For example, sampling variability, pollster to pollster variability, day to day variability, and election to election variability. One of the most successful approaches used for this are **hierarchical models(계층적 모델)**, which can be explained in the context of Bayesian statistics.
...
prev <- 0.00025
N <- 100000
outcome <- sample(c("Disease","Healthy"), N, replace = T, 
                  prob = c(prev, 1 - prev))
```
...
$$\ \frac{\mbox{Pr}(\mbox{두 애기 걍 사망} \mid \mbox{어미가 살인마} ) \mbox{Pr}(\mbox{어미가 살인마})}{\mbox{Pr}(\mbox{두 애기 걍 사망})}$$
...
```{r}
#밑에 seq 바로옆 숫자 0.05로 하면 왜 안될까?
var <- seq(0.005, 0.05, len = 100)
func <- function(tau){
  A <- sigma ^ 2 / (sigma^2 + tau^2)
  sd <- sqrt(1 / (1/sigma^2 + 1/tau^2))
  x <- A * mu + (1 - A) * Y
...
resids %>% filter(grade %in% c("A+","A","A-","B+") | is.na(grade)) %>%
  ...
  theme(axis.text.x = element_text(angle = 90, hju = 1)) +
  geom_boxplot() + 
  geom_point()
```
