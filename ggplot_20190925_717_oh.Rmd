...
3\. __Aesthetic mapping__: The plot uses several visual cues to represent the information provided by the dataset. The two most important cues in this plot are the point positions on the x-axis and y-axis, which represent population size and the total number of murders, respectively. Each point represents a different observation, and we _map_ data about these observations to visual cues like x- and y-scale. Color is another visual cue that we map to region. We refer to this as the __aesthetic mapping__ component. How we define the mapping depends on what __geometry__ we are using. 

We also note that:

1\. The points are labeled with the state abbreviations.
2\. The range of the x-axis and y-axis appears to be defined by the range of the data. They are both on log-scales. 
3\. There are labels, a title, a legend, and we use the style of The Economist magazine.
...
## 4.6 Global versus local aesthetic mappings
...
## 4.10 Annotation, shapes, and adjustments
...
## 4.11 Add-on packages {#add-on-packages}
...
## 4.13 Quick plots with `qplot` {#qplot}
...
```{r}
murders %>% ggplot(aes(population, total)) + geom_label(aes(population, total), label = murders$abb)
```
...
murders %>% ggplot(aes(population, total)) + 
  geom_label(aes(population, total), label = murders$abb, color = "blue")
```
...
```{r}
murders %>% ggplot(aes(population, total)) + 
  geom_label(aes(population, total, col = region), label = murders$abb)
```
