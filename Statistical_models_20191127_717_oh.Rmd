---
title: "Statistical_models_20191127_717_oh"
...
polls <- map_df(Ns, function(N) {
  ...
  x_hat <- mean(x)
  se_hat <- sqrt(x_hat * (1 - x_hat) / N) # sqrt(mean(x)*(1-mean(x)) / N)
...
```{r simulated-polls, message=FALSE}
...
```{r}
mean(x_s) + c(-1, 1)*qnorm(1 - 0.05/2) * sd(x_s) / sqrt(50)
ci <- c(qnorm(0.025, mean(x_s), std.error(x_s)), qnorm(0.975, mean(x_s), std.error(x_s)))
ci
```
...
polls %>% ggplot(aes(pollster, spread)) +
  geom_boxplot() +
  geom_sina(aes(color = pollster), show.legend = F)
```
...
$$
\mbox{E}[Y_{1,j}] = \mbox{E}[d + b_1 + \varepsilon_{i,j}] = 
d+b_1+\mbox{E}[\varepsilon_{i,j}] = d + b_1
$$
...
res <- polls %>% group_by(pollster) %>% 
  summarize(avg_spread = mean(spread), sd_spread = sd(spread), N = n()) 
res
#
estimate <- res$avg_spread[2] - res$avg_spread[1]
se_hat <- with(res, sqrt(sd_spread[2]^2/N[2] + sd_spread[1]^2/N[1]))
#
